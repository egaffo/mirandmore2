...
cutadapt_cmd = '''cutadapt $( -j $CPUS $) -a $ADAPTER --no-indels -n 10 -O $ADPT_OVERLAP '''\
               '''--quality-base $QUALITY_ENCODING -m $MIN_LEN -M MAX_LEN --trimmed-only '''\
               '''--too-long-output ${TARGETS[1]} $CUTADAPT_EXTRA_PARAMS -o ${TARGETS[0]} '''\
               '''${SOURCES[0]} > ${TARGETS[2]}'''
